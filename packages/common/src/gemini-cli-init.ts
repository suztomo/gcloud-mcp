...
export const initializeGeminiCLI = async (pkg_name: string, pkg_version: string, dirname: string) => {
  ...
  // Create directory
  const extensionDir = join(cwd, '.gemini', 'extensions', pkg_name);
  ...
  const extensionJson = {
    ...
    mcpServers: {
      gcloud: {
        command: 'npx',
        args: ['-y', `github:googleapis/gcloud-mcp ${pkg_name}`],
      },
    },
  };
  ...
  // Create GEMINI.md
  const geminiMdSrcPath = join(dirname, '../GEMINI-extension.md');
